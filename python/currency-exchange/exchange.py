...
def get_number_of_bills(amount: float, denomination: int):
    ...
    return int(round(amount / denomination, 0))
...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int):
    ...
    actual_budget: float = exchange_money(budget, exchange_rate + (float(exchange_rate * spread / 100 )))

    return actual_budget - get_leftover_of_bills(actual_budget, denomination)
